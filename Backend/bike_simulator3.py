...
class bike_simulator3:
    ...
    def load_data(self, directorios: list[str] = None, basepath="data"):
        ...
        return nearest_stations_idx, nearest_stations_distance, initial_movements, real_movements, \
        ...
        for movement in real_movements:  # Para cada movimiento.
            ...
            if movement.real == False:
                        ...
                        if tipoPeticion==Constantes.PETICION_DEJAR_BICI:
                            ...
                            if peticionesNoResueltas[movement.index] != 0:
                                matrices[Constantes.PETICIONES_NORESUELTAS_FICTICIAS_DEJAR_BICI].add_row(
                                    [int(movement.time)] + peticionesNoResueltas)
                        else:
                ...
                while amount_to_move > 0:  # Mientras existan movimientos a realizar:
                    ...
                    #Para contar las peticiones y no resueltas debo de calcularlo en la primera iteracion del algoritmo.

                    # Si no es la misma estación que la que he solicitado...
                    if not movement.index == nearest_station:  # Es decir, no estoy en el caso de que se ingresen a la misma estacion que el origen.
                        nPeticionesNoResueltas[movement.index] += asignadas
                        ##Corregir esto, tengo que comprobar que el amount to move es mayor que 0 arriba if.
                        if amount_to_move <= 0:  # Necesario para introducir los datos SOLO una vez.
                            if tipoPeticion == Constantes.PETICION_DEJAR_BICI:
                                matrices[Constantes.KMS_DEJAR_BICI].add_row([int(movement.time)] + listakms) #Revisar esto
                        ...
                        if tipoPeticion == Constantes.PETICION_DEJAR_BICI:
                            matrices[Constantes.PETICIONES_RESUELTAS_SOLTAR_BICI].add_row(
